...
describe("PasswordEntropyLab", () => {
  ...
  it("shows Very Strong strength and matching entropy bits for a long mixed password", () => {
    render(
      <PasswordEntropyLab password="Tr0ub4dor&3xtra-long!" setPassword={vi.fn()} />
    );
  ...
  it("updates strength when the password prop changes", () => {
    const { rerender } = render(
      <PasswordEntropyLab password="a" setPassword={vi.fn()} />
    );
    expect(screen.getByText("VERY WEAK")).toBeInTheDocument();

    rerender(
      <PasswordEntropyLab password="Aa1!Aa1!Aa1!Aa1!Aa1!" setPassword={vi.fn()} />
    );
    expect(screen.getByText("VERY STRONG")).toBeInTheDocument();
  });
});
